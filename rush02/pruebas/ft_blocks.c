...
/*   Updated: 2019/10/20 03:45:54 by bmoran-f         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	ft_blocks(char *str, char *buf, char **argv)
{
	int i;
	int j;
	int length;
	...
	length2 = l_first_block(argv[1]);
	ft_print_3_blocks(argv[1], buf, i, length2);
	...
	while (i < length)
	{
		...
		j = 0;
		ft_print_3_blocks(argv[1], buf, i, 3);
		while (j < 3)
		{
			//write(1, &str[i], 1); //
			j++;
